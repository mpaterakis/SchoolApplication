...
public class FindUsers extends JDialog {

    public FindUsers(Connection con, boolean modal, AbstractTableModel etb) {
        this.etb = etb;
        this.con = con;
        initComponents();
    }
    ...
    private void doSearch() {
        ...
        if (!search4.equals(null)) {
            etb = new UsersSearchModel(con, search4, false);
            t.setModel(etb);
            t.repaint();
        }
    }
    ...
    private void doShowAll() {
        etb = new UsersSearchModel(con, null, true);
        t.setModel(etb);
        t.repaint();
    }
