...
public class FindTeachers extends JDialog {

    public FindTeachers(Connection con, boolean modal, AbstractTableModel etb) {
        this.etb = etb;
        this.con = con;
        initComponents();
    }
    ...
    private void doSearch() {
        ...
        if (!search4.equals(null)) {
            etb = new TeachersSearchModel(con, search4, false);
            t.setModel(etb);
            t.repaint();
        }
    }
    ...
    private void doShowAll() {
        etb = new TeachersSearchModel(con, null, true);
        t.setModel(etb);
        t.repaint();
    }
